import { buildOnchainMetadata, parseToJSON } from "./utils/jetton-helpers";
import {
    Blockchain,
    SandboxContract,
    TreasuryContract,
...
import { Address, beginCell, fromNano, StateInit, toNano, Cell } from "@ton/core";
import { TonClient4 } from "@ton/ton";
import { printSeparator } from "./utils/print";

// -------- Contract SDK --------
import { SenseInCryptoJetton, Mint, TokenTransfer } from "./output/SenseInCryptoJetton_SenseInCryptoJetton";
...
describe("contract", () => {
    ...
    let token: SandboxContract<SenseInCryptoJetton>;
    ...
    beforeAll(async () => {
        ...
        token = blockchain.openContract(await SenseInCryptoJetton.fromInit(deployer.address, content));

        // Send Transaction
        const mintMessage: Mint = {
            $$type: "Mint",
            amount: toNano(1), // 1 SICC
            receiver: deployer.address,
        };
        const deployResult = await token.send(deployer.getSender(), { value: toNano("10") }, mintMessage);
    ...
    it("Test: whether contract deployed successfully", async () => {
        ...
        let data = await token.getGetJettonData()
        console.log(data.content.toString())
        let parsed = parseToJSON(data.content.toString())
        console.log(parsed)
    ...
    //     const jettonRoot = blkch.openContract(await SenseInCryptoJetton.fromInit(player.address, content));
